...
public class RegexParser {
	...
	public static ParseTree parse(String regex, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
        PushbackReader reader = new PushbackReader(new StringReader(regex), regex.length());
	...
	// <rexp> -> <rexp1> <rexp¡¯>
		private static void rexp(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
		...
		// <rexp1> -> <rexp2> <rexp1¡¯> | E
		private static void rexp1(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
		...
		private static void rexpPrime(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			Character next = peek(reader);
			if(new Character('|').equals(next)) {	
				reader.read(); // consume the character
				ParseTree rexpPrimeNode = new ParseTree(NonterminalSymbol.REXP_PRIME);
...
//				te.getNextToken();
		...
		// <rexp1¡¯> -> <rexp2> <rexp1¡¯>  | E        
		private static void rexp1Prime(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {	
			Character next = peek(reader);
...
//			
			...
			if (next.equals('(') || peekReChar(reader) != null || next.equals('.') || next.equals('[') || next.equals('$')) {
				ParseTree rexp1PrimeNode = new ParseTree(NonterminalSymbol.REXP1_PRIME);
...
//			}
		...
		private static void rexp2(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			...
			if (Character.valueOf('(').equals(next)) {
				...
				if (!Character.valueOf(')').equals(peek(reader))) {
					throw new ParseException("Rexp2: Expected ), got " + peek(reader));
				} // otherwise
				rexp2Node.addChild(new ParseTree(new TerminalSymbol(")")));
		...
		// <rexp2-tail> -> * | + | E
		private static void rexp2_tail(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {		
			Character next = peek(reader);
			if (next != null && (next.equals('*') || next.equals('+'))) {
				reader.read(); // consume char
				ParseTree rexp2TailNode = new ParseTree(NonterminalSymbol.REXP2_TAIL);
			    rexp2TailNode.addChild(new ParseTree(new TerminalSymbol(next.toString())));
		...
		// <rexp3> -> <char-class>  | E   
		private static void rexp3(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			Character next = peek(reader);
...
//			}
		...
		private static void char_class(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			Character next = peek(reader);
			System.out.println("charclass: peeking at " + next);
...
//			return;
		...
		private static void char_class1(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			//Token ahead = te.peekNextToken();
			Character next = peek(reader);
			System.out.println("charclass1: peeking at " + next);
		...
		private static void char_set_list(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			ParseTree charSetListNode = new ParseTree(NonterminalSymbol.CHAR_SET_LIST);
			if (peekClsChar(reader) != null) {
				...
				throw new ParseException("Char-set-list: Expected cls char or ], got " + peek(reader));
			}
...
//			}
		...
		private static void char_set(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {	
			String clsChar = peekClsChar(reader);
			if (clsChar == null) {
				throw new ParseException("Char-set: expected clsChar, got " + peek(reader));
			}
...
//	        char_set_tail(ahead);
		...
		private static void char_set_tail(ParseTree root,
				PushbackReader reader, Map<String, ParseTree> definedClasses) throws IOException, ParseException {
			Character next = peek(reader);
			if (!Character.valueOf('-').equals(next)) {
				return;
			}
			reader.read(); // consume
			ParseTree charSetTailNode = new ParseTree(NonterminalSymbol.CHAR_SET_TAIL);
			charSetTailNode.addChild(new ParseTree(new TerminalSymbol("-")));
			String clsChar = peekClsChar(reader);
			if (clsChar == null) {
				throw new ParseException("Char-set-tail: expected clsChar, got " + peek(reader));
			}
...
//		// <char-set-tail> -> - CLS_CHAR | E
//		private static void char_set_tail(Token start) throws ParseException {	
//			Token ahead = te.peekNextToken();
...
//		}
		...
		private static void exclude_set(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			Character next = peek(reader);
			if (!Character.valueOf('^').equals(next)) {
			...
			next = peek(reader);
			if (Character.valueOf(' ').equals(next)) { // spaces are optional here, but let's not make a node for them
			...
			if (!sb.toString().equals("] IN ")) {
				throw new ParseException("Exclude-set: expected ] IN, got " + sb.toString());
			}
			excludeSetNode.addChild(new ParseTree(new TerminalSymbol("]")));
...
//	         exclude_set_tail();
		...
		// <exclude-set-tail> -> [<char-set>]  | <defined-class>
		 private static void exclude_set_tail(ParseTree root, PushbackReader reader, Map<String, ParseTree> definedClasses) throws ParseException, IOException {
			 Character next = peek(reader);
			 ParseTree excludeSetTailNode = new ParseTree(NonterminalSymbol.EXCLUDE_SET_TAIL);
			 if (Character.valueOf('[').equals(next)) {
				 ...
				 if (!Character.valueOf(']').equals(next)) {
					 throw new ParseException("Exclude-set-tail: Expected ], got "  + next);
				 }
...
//		 
//	    private static void defined_class(Token token, boolean flag) throws ParseException {    
//	         return;         
//	     }
	    
		
		private static void defined_class(ParseTree root,
				PushbackReader reader, Map<String, ParseTree> definedClasses) throws IOException, ParseException {
			// find longest match of class name in reader, then append that parse tree to root
			// alternatively just plop the whole class name in there and deal with it later. probably best
			System.out.println("defined classes: " + definedClasses.keySet());
			int input;
			...
			if (name == null) {
				throw new ParseException("Defined-class: got " + nameBuilder.toString());
			}
...
//	     }
	     private static String peekClsChar(PushbackReader reader) throws IOException {
	         ...
	         if (next == null) {
	        	 return null;
	         } else if (isAsciiPrintable(next) && !isForbiddenInCls(next) && !Character.valueOf('$').equals(next)) {
	    	 ...
	    	 if (oneAfterNextInt != -1 && next.equals('\\') && isForbiddenInCls(oneAfterNext)) {
		...
		private static String peekReChar(PushbackReader reader) throws IOException {
	    	 Character next = peek(reader);
	    	 // first check if it's ascii printable (but not space, \, *, +, ?, |, [, ], (, ), ., ' and ")
	         if (next == null) {
	        	 return null;
	         } else if (isAsciiPrintable(next) && !isForbiddenInRe(next) && !Character.valueOf('$').equals(next)) {
	        	 return next.toString();
	         }
	    	 ...
	    	 if (oneAfterNextInt != -1 && next.equals('\\') && isForbiddenInRe(oneAfterNext)) {
	    		 return next.toString() + oneAfterNext.toString();
	    	 }
